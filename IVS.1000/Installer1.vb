    ...
    Private Sub Installer1_AfterInstall(sender As Object, e As System.Configuration.Install.InstallEventArgs) Handles Me.AfterInstall
        ...
        Try
            ...
            Select Case strDatabaseType
                ...
                Case 1
                    'Access
                    strConnectionString = String.Format("Provider=Microsoft.ACE.OLEDB.12.0;Data Source={0}\IVS.1000.accdb", strAccessDatabase)
                ...
                Case 3
                    'Manual
                    strConnectionString = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=C:\IVS.1000.accdb"
                    strProviderName = "System.Data.OleDb"
            End Select

            ' MyAppLog.WriteToLog("IVS.Setup() Connection String: " & strConnectionString)
            ...
            'strProcess = strTargetDirectory & "Drivers\vcredist_x86.exe"
            'MyAppLog.WriteToLog("IVS.Setup() Starting process: " & strProcess)
            'p = System.Diagnostics.Process.Start(strProcess, "/passive")
            'p.WaitForExit()
            'MyAppLog.WriteToLog("IVS.Setup() Finished process: " & strProcess)
            ...
            If strDatabase = 2 Then
                ...
                Dim strSourceFile As String = String.Format("{0}IVS.1000.accdb", strTargetDirectory)
                Dim strDestinationDirectory As String = strAccessDatabase
                Dim strDestinationFile As String = String.Format("{0}\IVS.1000.accdb", strAccessDatabase)
